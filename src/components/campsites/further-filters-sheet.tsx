...
/** "Weitere Filter" als Vollbild-Pop-up -- anders als MobileFilterSheet
 * (siehe src/components/mobile-filter-sheet.tsx) bewusst auf ALLEN
 * Bildschirmgroessen aktiv, nicht nur mobil: Land und Elektromobilitaet
 * stehen jetzt direkt auf der Seite (siehe quick-filters.tsx), der Rest
 * (Suche + uebrige Merkmals-Kategorien) bleibt dahinter verborgen, egal ob
 * Desktop oder Handy -- eigene Komponente statt MobileFilterSheet
 * wiederzuverwenden, damit dessen `lg:hidden`-Verhalten fuer /ladepunkte
 * unangetastet bleibt. */
export function CampsiteFurtherFiltersSheet({
  children,
  activeFilterCount,
}: {
  children: ReactNode;
