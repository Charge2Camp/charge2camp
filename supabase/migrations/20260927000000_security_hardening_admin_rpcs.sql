-- Sicherheits-Audit (intensive Plattform-Pruefung): zwei Funde behoben.
--
-- 1) enrich.app_user speicherte bisher die echte E-Mail-Adresse in
--    display_name (siehe enrich.submit_trailer_report, admin/.../
--    ladestationen/[id]/actions.ts overrideTrailerSuitability). Durch die
--    generische "readable by everyone"-Policy aus
--    20260915000000_data_layer_api_exposure.sql (gilt fuer ALLE Tabellen in
--    core/enrich, ohne Ausnahme fuer identitaetsbezogene Daten) war das
--    OEFFENTLICH -- auch fuer anon, ganz ohne Login -- ueber die Supabase-
--    API abrufbar (`{SUPABASE_URL}/rest/v1/app_user?select=*`).
--    display_name wird in der App an keiner Stelle gelesen/angezeigt (nur
--    geschrieben, siehe Code-Grep) -- komplett aus display_name entfernen
--    statt nur zu verstecken, und die Tabelle fuer anon/authenticated ganz
--    sperren (kein legitimer Lesefall ueber die oeffentliche API; die
--    einzigen Schreibpfade sind SECURITY DEFINER-Funktionen bzw. der
--    Admin-Service-Role-Client, die RLS/GRANTs ohnehin umgehen).
update enrich.app_user set display_name = null where display_name is not null;

drop policy if exists "app_user readable by everyone" on enrich.app_user;
revoke select on enrich.app_user from anon, authenticated;

-- 2) enrich.moderate_trailer_report / enrich.submit_campsite_charging /
--    core.research_queue / core.run_quality_checks sind an die Postgres-
--    Rolle "authenticated" granted (fuer PostgREST-RPC-Zugriff noetig),
--    pruefen selbst aber NICHT, ob der Aufrufer Admin ist -- die
--    Admin-Pruefung (profiles.is_admin) lag bisher AUSSCHLIESSLICH im
--    jeweiligen Next.js-Route-Handler (siehe Kommentare in
--    20260918000000_enrichment_endpoints.sql: "die eigentliche
--    Berechtigungspruefung macht der jeweilige Route Handler VOR dem
--    RPC-Aufruf"). Jeder eingeloggte Nutzer konnte die Funktionen dadurch
--    per direktem PostgREST-RPC-Aufruf (unter Umgehung der Next.js-Route)
--    aufrufen -- z. B. beliebige Anhaengertauglichkeits-Meldungen
--    genehmigen/ablehnen. Fix: dieselbe profiles.is_admin-Pruefung jetzt
--    zusaetzlich IN der Funktion selbst (Defense in Depth) -- die
--    bestehenden Route Handler funktionieren unveraendert weiter (ihr
--    eigener Check bleibt als schnellerer 403 vor dem RPC-Aufruf
--    erhalten), nur der Umgehungsweg per direktem RPC-Aufruf wird
--    geschlossen.
...
-- Vorher "language sql" ohne security definer (Kommentar: "core.campsite/
-- enrich.campsite_charging sind fuer authenticated bereits per
-- SELECT-Policy lesbar") -- das galt fuer die GELESENEN Tabellen, nicht
-- fuer die jetzt neu hinzugekommene Admin-Pruefung selbst. security
-- definer + explizite Sprache plpgsql (fuer die IF-Pruefung noetig)
-- stellt sicher, dass der Admin-Check unabhaengig von GRANTs auf
-- public.profiles zuverlaessig funktioniert.
...
-- Vorher "language sql" -- gleicher Grund wie bei core.research_queue()
-- oben (IF-Pruefung braucht plpgsql), Rest der Abfragen unveraendert nur
-- in ein "return query select * from ( ... ) t" gewrappt.
