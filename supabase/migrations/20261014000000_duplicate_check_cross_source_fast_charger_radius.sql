-- Der Dubletten-Check core.run_quality_checks() -> duplicate_charge_points
-- verlangte bisher exakt gleichen Betreiber-String (a.operator IS NOT
-- DISTINCT FROM b.operator) + 25m Radius. Das verfehlt systematisch echte
-- Dubletten zwischen unterschiedlichen Quellen: OCM und die Bundesnetzagentur
-- schreiben denselben Betreiber oft unterschiedlich (z. B. OCM
-- "Naturenergie (DE)" vs. BNetzA "naturenergie hochrhein AG", oder OCM
-- Platzhalter wie "NULL"/"(Business Owner at Location)" statt eines echten
-- Namens). Nutzeranalyse (2026-09-20): von 8069 OCM-Ladepunkten ganz ohne
-- core.connector-Zeilen (OCM selbst liefert dafuer kein Connections-Array)
-- haben 6519 (80,8%) eine BNetzA-Station mit echten Anschlussdaten im
-- Umkreis von 60m -- aber nur 48 davon exakt denselben Betreiber-String und
-- waeren damit vom bisherigen Check je erkannt worden.
--
-- Aenderung: der exakte Betreiber-Match entfaellt komplett (beide
-- Betreiber-Strings werden stattdessen mit ausgegeben, damit der Admin
-- selbst vergleichen kann -- DuplicateRow.operator wird zu operator_a/
-- operator_b, siehe admin/app/(dashboard)/ladestationen/dubletten/page.tsx).
-- Der Radius wird bei Schnellladern auf 100m angehoben (Nutzervorgabe) --
-- Schnelllade-Hubs (Raststaetten, Hubs mit mehreren Saeulen) haben real
-- groessere Abstaende zwischen zusammengehoerigen Punkten als normale
-- AC-Wallboxen, fuer die die bisherigen 25m weiterhin gelten.
--
-- "Schnellader" wird ueber core.charge_point_is_fast_charger() bestimmt:
-- mindestens ein Anschluss mit current_type='DC', sonst (nur relevant fuer
-- genau die Stationen ohne jede core.connector-Zeile, s.o.) Fallback auf
-- max_power_kw >= 43 -- empirisch gegen eine echte BNetzA-Datei geprueft:
-- "hat DC-Anschluss" stimmt in 98,8% der Faelle mit BNetzA's eigener
-- Klassifikation 'Schnellladeeinrichtung' ueberein.
create or replace function core.charge_point_is_fast_charger(p_id uuid)
returns boolean
language sql
stable
...
as $$
    select coalesce(
        (select bool_or(c.current_type = 'DC') from core.connector c where c.charge_point_id = p_id),
        (select max_power_kw >= 43 from core.charge_point where id = p_id)
    );
$$;

grant execute on function core.charge_point_is_fast_charger(uuid) to authenticated, service_role;
    ...
    select 'duplicate_charge_points', to_jsonb(t) from (
        ...
        join core.charge_point b
         ...
         and st_dwithin(
               a.geom, b.geom,
               case
                   when core.charge_point_is_fast_charger(a.id) or core.charge_point_is_fast_charger(b.id)
                   then 100
                   else 25
               end
             )
