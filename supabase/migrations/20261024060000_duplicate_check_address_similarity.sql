-- Fortsetzung von 20261024050000: von den verbleibenden ~1.200 Paaren mit
-- passendem Betreiber matcht der Grossteil (~1.096) keine der Tier-Regeln
-- und ist per Stichprobe ueberwiegend KEIN Duplikat, sondern derselbe
-- Netzbetreiber mit mehreren echten, unterschiedlichen Standorten in
-- derselben Stadt (z. B. "MOBIVE | Bourganeuf | Place du Champ de Foire"
-- vs. "... | Rue de L'Etang"). Die "Near Misses" mit der hoechsten echten
-- Trefferwahrscheinlichkeit sind Paare mit Adress-Aehnlichkeit knapp UNTER
-- der Tier-Schwelle (z. B. "Circle K ... Im Heidegrund Süd 1" vs. "...
-- Autohof Im Heidegrund-Süd 0", addr_sim=0.60, knapp unter tier8s 0.7) --
-- aktuell verstreut irgendwo in der zufaelligen Reihenfolge der ~1.200
-- Treffer statt priorisiert oben zu stehen.
--
-- Fix: addr_sim (normalisierte Adress-Trigram-Aehnlichkeit, dieselbe
-- Funktion wie in den Tier-Regeln) wird jetzt mit in core.
-- quality_check_cache gespeichert -- admin/app/(dashboard)/ladestationen/
-- dubletten/page.tsx bekommt damit eine neue Sortieroption "Adress-
-- Aehnlichkeit (hoechste zuerst)".
...
begin
    ...
    -- Betreiberkonflikt-Filter (20261024050000) + addr_sim fuer die neue
    -- Sortieroption im Admin-Dashboard (20261024060000).
    insert into core.quality_check_cache (check_name, data)
...
comment on function core.refresh_quality_checks() is
  'Befuellt core.quality_check_cache neu (TRUNCATE + INSERT, eine Zeile je Treffer). duplicate_charge_points schliesst Paare mit eindeutigem Betreiberkonflikt aus (20261024050000) und traegt addr_sim fuer die Adress-Aehnlichkeits-Sortierung im Admin-Dashboard (20261024060000). Braucht bei aktueller Groesse mehrere Sekunden bis niedrige Minuten -- nur ueber pg_cron aufrufen (core.refresh_all_quality_data()), nie synchron aus der Admin-UI.';

-- Einmaliger Nachlauf, damit die neue Sortieroption sofort Daten hat.
