-- Fortsetzung von 20261022020000: Nutzeranfrage, Paare mit Betreiber
-- "ladenetz.de" (bundesweites Roaming-Dach fuer viele deutsche Stadtwerke,
-- kein eigener Standortbetreiber) gezielt zu pruefen. Ergebnis (161 offene
-- Paare mit ladenetz.de auf einer Seite): bei den staerksten Kandidaten ist
-- die Adresse nach Normalisierung EXAKT identisch (nicht nur aehnlich),
-- der Abstand liegt aber bei 20-99,8m -- knapp ausserhalb von Tier 8s
-- 20m-Grenze. Verallgemeinert (alle offenen Paare, nicht nur ladenetz.de):
-- 281 Paare mit exakt identischer normalisierter Adresse, alle > 20m
-- entfernt, max. 99,8m -- bereits durch den urspruenglichen Kandidaten-
-- Radius von core.charge_point_duplicate (<=100m fuer Schnelllader) nach
-- oben begrenzt.
--
-- Stichprobe (20 Paare mit dem groessten Abstand, 78-99,8m): durchweg
-- grossflaechige, plausible gemeinsame Standorte (Autohoefe, Einkaufs-
-- zentren/Fachmaerkte, Autohaeuser, Firmengelaende) -- z. B. "Autohof
-- Elbert" (Nomadpower) / "Aral Pulse" (BP Europa SE) an derselben
-- Bahnhofstrasse 53, 87,8m; "BMW Niederlassung Hannover am EXPOPark"
-- (ChargeNow) / "enercity AG" an derselben Lissabonner Allee 3, 81,5m.
-- Eine EXAKT identische, nicht-generische Adresse (Strasse + Hausnummer)
-- ist ein staerkeres Signal als jeder Abstands- oder Betreiber-Abgleich --
-- deshalb neue Tier 9 ohne Abstands- oder Betreiber-Bedingung (Abstand ist
-- durch den Kandidaten-Radius der Quelltabelle ohnehin schon <= 100m).
...
comment on function core.auto_merge_charge_point_duplicates_batch(int) is
  'Wie core.auto_merge_charge_point_duplicates() (20261021000000), aber auf p_limit Paare pro Aufruf begrenzt (PostgREST/Pooler-Statement-Timeout ~8-9s) -- wiederholt aufrufen, bis 0 Zeilen zurueckkommen. Tiers 1-3 klassisch, 4/5 Geo-Toleranz, 6/7 lose/fehlende Betreiber-Aehnlichkeit mit Adress-Bestaetigung, 8 sehr starke Adress-Uebereinstimmung (>=0.7, <=20m), 9 (20261022030000) exakt identische normalisierte Adresse ersetzt Abstands- UND Betreiber-Abgleich komplett.';
