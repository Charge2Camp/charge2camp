-- Fix fuer 20261021000000/20261021010000: v_run_started wurde mit
-- clock_timestamp() gesetzt (tatsaechliche Uhrzeit beim Erreichen dieser
-- Zeile), core.charge_point_duplicate_auto_merge_log.merged_at dagegen mit
-- dem Spalten-Default now() befuellt -- now() liefert in Postgres den
-- TRANSAKTIONSSTART-Zeitpunkt zurueck, nicht die aktuelle Uhrzeit, und
-- liegt deshalb IMMER vor dem spaeteren clock_timestamp()-Aufruf in
-- derselben Funktion. Folge: "where l.merged_at >= v_run_started" traf nie
-- eine Zeile -- die Funktion mergte korrekt (bestaetigt: erster Testlauf
-- gegen Prod hat 150 Zeilen in core.charge_point_duplicate_auto_merge_log
-- geschrieben), gab aber immer eine leere Ergebnistabelle zurueck statt der
-- Tier-Zaehlung. now() statt clock_timestamp() liefert in derselben
-- Transaktion denselben Wert wie der Insert-Default und behebt das.
