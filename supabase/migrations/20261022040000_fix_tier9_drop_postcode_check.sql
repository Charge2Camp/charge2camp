-- Fix fuer 20261022030000: Tier 9 verlangte zusaetzlich passende PLZ (falls
-- beide vorhanden), obwohl das Kernkriterium (exakt identische normalisierte
-- Adresse) bereits eindeutig ist. Gefunden bei der ladenetz.de-Detailpruefung
-- (Nutzeranfrage): Paare wie "Rosenheimer Platz 6" == "Rosenheimer Platz 6"
-- (Strasse+Hausnummer exakt identisch) wurden NICHT gemergt, weil die PLZ auf
-- beiden Seiten vorhanden, aber unterschiedlich war. Bei identischem
-- Strassennamen+Hausnummer ist eine abweichende PLZ ein Datenfehler auf einer
-- Seite, kein Hinweis auf unterschiedliche Standorte -- zumal die Kandidaten
-- ohnehin schon durch den geografischen Radius von core.charge_point_duplicate
-- (physische Naehe) vorgefiltert sind, PLZ-Text ist da redundant und teils
-- fehlerhaft. PLZ-Bedingung aus Tier 9 entfernt.
