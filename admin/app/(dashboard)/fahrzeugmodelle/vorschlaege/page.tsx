...
import type { VehicleModelSuggestion } from "@/lib/types";
import { rejectVehicleModelSuggestion } from "./actions";

export default async function VehicleModelSuggestionsPage() {
  ...
  const pending = (suggestions ?? []) as VehicleModelSuggestion[];
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="mt-1 text-sm text-text-muted">
          Von Nutzern über &quot;Mein Gespann&quot; vorgeschlagene, bei uns fehlende Fahrzeugmodelle -- {pending.length}{" "}
          offen.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        {pending.map((s) => (
          <div key={s.id} className="flex flex-col gap-2 rounded-md border border-line bg-card p-3 text-sm">
            ...
            <p className="text-text-muted">
              {[
                s.battery_capacity_kwh != null && `${s.battery_capacity_kwh} kWh`,
                s.range_km != null && `${s.range_km} km Reichweite`,
                s.charging_power_kw != null && `${s.charging_power_kw} kW Ladeleistung`,
              ]
                .filter(Boolean)
                .join(" · ") || "Keine weiteren technischen Angaben."}
            </p>
            <div className="flex gap-2">
              ...
              <form action={rejectVehicleModelSuggestion.bind(null, s.id)}>
                <button
                  type="submit"
